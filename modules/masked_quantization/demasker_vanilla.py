...
class VanillaDemasker(nn.Module):
    def __init__(self, codebook_dim, output_dim, height_and_width, n_layer, mask_init_value = 0.02, ratio_dim=1):
        super().__init__()
        self.ratio_dim = ratio_dim
        self.output_dim = output_dim
        self.codebook_dim = codebook_dim
        self.hw = height_and_width
        self.total_code_num = height_and_width * height_and_width*self.ratio_dim
    ...
    def forward(self, sampled_quant, remain_quant, sample_index, remain_index = None, mask = None):
        ...
        full_embedding = torch.zeros((batch_size, self.codebook_dim, self.total_code_num)).to(sampled_quant.device)

        for i in range(batch_size):
            full_embedding[i, :, sample_index[i]] = sampled_quant[i]
            full_embedding[i, :, remain_index[i]] = self.mask_token

        full_embedding = rearrange(full_embedding, "B C (H W) -> B C H W", H=self.hw, W=self.hw*self.ratio_dim)
