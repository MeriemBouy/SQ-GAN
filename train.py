...
now = datetime.datetime.now().astimezone(Berlin).strftime("%m-%dT%H-%M-%S")
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-m", "--mode", type=str, default="ssm", choices=["ssm", "img", "all"], help="speciy which training step you want to perform",)
    parser.add_argument("-c", "--ckpt_path_parts", type=str, default=None, help="specify the repo where the different trained parts have been stored",)
    parser.add_argument("-sp", "--split_path", type=str, default=None, help="specify the path where to store the splitted submodels",)
    ...
    parser.add_argument("-p", "--project", help="name of new or path to existing project", default="AdaptiveVectorQuantization")
    parser.add_argument("--save_n", default=3, type=int, help="save top-n with monitor or save every n epochs without monitor")
    # parser.add_argument("--accumulate_grad_batches", default=4, type=int, help="accumulate_grad_batches")
...
def split_submodules(model, split_dir, mode_str):
    """
    Saves relevant submodules into individual .ckpt files, depending on mode_str.
    Then exits.
    """
    os.makedirs(split_dir, exist_ok=True)

    # Decide which submodules to save based on the mode
    if mode_str == "ssm":
        modules_to_save = {
            "encoder_semantic": model.encoder_semantic,
            "decoder_semantic": model.decoder_semantic,
            "masker_semantic": model.masker_semantic,
            "demasker_semantic": model.demasker_semantic,
            "quantize_semantic": model.quantize_semantic,
        }
    elif mode_str == "img":
        modules_to_save = {
        ...
        print(f"Mode '{mode_str}' is neither 'ssm' nor 'img'. Skipping splitting.")
        sys.exit(0)

    for name, submodule in modules_to_save.items():
        if submodule is None:
            print(f"Skipping {name} as it is None.")
            continue
        sd = submodule.state_dict()
        ckpt_path = os.path.join(split_dir, f"{name}.ckpt")
        torch.save(sd, ckpt_path)
        print(f"Saved submodule '{name}' to '{ckpt_path}'.")

    print("Submodule splitting complete. Exiting now.")
    sys.exit(0)  # KILL the code after splitting
...
if __name__ == "__main__":
    ...
    # Override mode and ckpt_path_parts if provided via CLI
    if opt.mode:
        config.model.params.mode = opt.mode  # Ensure model params exist before overriding

    if opt.ckpt_path_parts:
        config.model.params.ckpt_path_parts = opt.ckpt_path_parts  # Ensure the structure exists before overriding
    ...
    # ---------------------------------------------------------------
    # If user only wants to split the submodules, do it and then exit
    # ---------------------------------------------------------------
    if opt.split_path:
        print(f"Splitting submodules to folder: {opt.split_path}")
        # This will KILL the code after saving submodules
        split_submodules(model, opt.split_path, mode_str=config.model.params.mode)
    ...
    trainer_config["accumulate_grad_batches"] = 4
    ...
    # ---------------------------------------------------------------
    # If this is the final training then we want to avoid the finetuning of G_s
    # ---------------------------------------------------------------
    if config.model.params.mode == "all":  
        for name in [f.split(".")[0] for f in os.listdir(opt.ckpt_path_parts) if f.endswith('semantic.ckpt')]:
            getattr(model, name).eval()
            for _, param in getattr(model, name).named_parameters():
